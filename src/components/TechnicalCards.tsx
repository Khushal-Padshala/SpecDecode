    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {concepts.map((c, idx) => (
          <div
            key={idx}
            className="rounded-xl border border-pink-200/80 bg-white/95 p-5 space-y-3 hover:border-pink-300 transition-colors shadow-sm"
          >
            <div className="flex items-center justify-between">
              <div className="p-2 rounded-lg bg-pink-50 border border-pink-200">
                {c.icon}
              </div>
              <span className="text-[10px] font-mono uppercase tracking-wider text-slate-700 font-medium px-2 py-0.5 rounded bg-pink-50 border border-pink-200">
                {c.tag}
              </span>
            </div>
